    ...
    partial class ILImporter
    {
        ...
        private void InitialPass()
        {
            ...
            while (_currentOffset < _ilBytes.Length)
            {
                ...
                OpCodes opCode = (OpCodes)ReadILByte();
            ...
            again:
                switch (opCode)
                {
                    ...
                    case OpCodes.constrained:
                        ...
                        continue;
                    case OpCodes.tail:
                    case OpCodes.volatile_:
                    case OpCodes.readonly_:
                        previousWasPrefix = true;
                        continue;
                    ...
                    case OpCodes.prefix1:
                        opCode = (OpCodes)(0x100 + ReadILByte());
                        goto again;
                    default:
                        break;
                }
        ...
        void CheckIsComparable(StackValue a, StackValue b, OpCodes op)
        {
        ...
        void CheckDelegateCreation(StackValue ftn, StackValue obj)
        {
            ...
            {
                if (GetOpcodeAt(delegateStart) != OpCodes.ldftn)
                {
                    VerificationError(VerifierError.DelegatePattern);
                    return;
                }
            ...
            {
                if (GetOpcodeAt(delegateStart) != OpCodes.dup ||
                    GetOpcodeAt(delegateStart + 1) != OpCodes.ldvirtftn)
                {
                    VerificationError(VerifierError.DelegatePattern);
                    return;
                }
        ...
        OpCodes GetOpcodeAt(int instructionOffset)
        {
            var opCode = (OpCodes)_ilBytes[instructionOffset];
            if (opCode == OpCodes.prefix1)
                opCode = (OpCodes)(0x100 + _ilBytes[instructionOffset + 1]);
        ...
        void ImportCasting(OpCodes opcode, int token)
        {
        ...
        void ImportCall(OpCodes opcode, int token)
        {
            FatalCheck(opcode != OpCodes.calli, VerifierError.Unverifiable);
            ...
            {
                if (HasPendingPrefix(Prefix.Constrained) && opcode == OpCodes.callvirt)
                {
            ...
            if (opcode == OpCodes.newobj && methodType.IsDelegate)
            {
            ...
            if (tailCall)
            {
                ...
                Check(_currentOffset < _ilBytes.Length && (OpCodes)_ilBytes[_currentOffset] == OpCodes.ret, VerifierError.TailRet);
            }
        ...
        void ImportLdFtn(int token, OpCodes opCode)
        {
            MethodDesc method = ResolveMethodToken(token);
        ...
        void ImportBranch(OpCodes opcode, BasicBlock target, BasicBlock fallthrough)
        {
            switch (opcode)
            {
                case OpCodes.br:
        ...
        void ImportBinaryOperation(OpCodes opcode)
        {
            var op1 = Pop();
            var op2 = Pop();
        ...
        void ImportShiftOperation(OpCodes opcode)
        {
            var shiftBy = Pop();
            var toBeShifted = Pop();
        ...
        void ImportCompareOperation(OpCodes opcode)
        {
            var value1 = Pop();
            var value2 = Pop();
        ...
        void ImportUnaryOperation(OpCodes opCode)
        {
            var operand = Pop();
        ...
        void ImportUnbox(int token, OpCodes opCode)
        {
            var type = ResolveTypeToken(token);
        ...
        void ReportInvalidInstruction(OpCodes opcode)
        {
